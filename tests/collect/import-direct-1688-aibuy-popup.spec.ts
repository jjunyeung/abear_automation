...
const ATC_PATH = join(
  ...
  'atcs',
  'collect',
  'import-direct-1688-aibuy-popup.atc.yml',
);
...
const handlers: StepHandlers = {
  ...
  click_mall_verify_popup: async (page) => {
    const r = await verifySourcingMallPopup(page, '1688 AiBUY', /(^|\.)1688\.com$/i);
    if (r.ok) return { ok: true as const };
    return { ok: false as const, error_key: r.error_key as ErrorKey, message: r.message };
  },
};

test('상품 수집 - 1688 AiBUY 버튼 새 창 popup 검증', async ({ page }) => {
