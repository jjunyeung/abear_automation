...
const ATC_PATH = join(
  ...
  'atcs',
  'collect',
  'import-direct-tmall-popup.atc.yml',
);
...
const handlers: StepHandlers = {
  ...
  click_tmall_verify_popup: async (page) => {
    const r = await verifySourcingMallPopup(page, '티몰', /(^|\.)tmall\.com$/i);
    if (r.ok) return { ok: true as const };
    return { ok: false as const, error_key: r.error_key as ErrorKey, message: r.message };
  },
};

test('상품 수집 - 티몰 버튼 새 창 popup 검증', async ({ page }) => {
